...
{
    public class Gestion10ConfTQEService
    {
        ...
        public Gestion10ConfTQEList GetGestion10ConfTQEList(string empNum)
        {

            Gestion10ConfTQEList model = new Gestion10ConfTQEList();
            var items = (from d in _entities.Gestion10ConfTQE_Admin
                         join e in _entities.Gestion10ConfTQE on d.Disc_ID equals e.Disc_ID
            ...
            List<Gestion10ConfTQEItem> di = items.Select(i =>
            new Gestion10ConfTQEItem
            {
        ...
        public CommonModelGestion10ConfTQE GetDetailsForSelectedGroup(string selectedGroup)
        {
            var adGroupName = _entities.Gestion10ConfTQE_ADGroup
                                .Where
                                    (p => p.Disc_ID.ToString() == selectedGroup)
                                .Select
                                    (p => p.ADGroup_Write).FirstOrDefault();

            var availableToEmployeesMembersOfTmp = _entities.Gestion10ConfTQE_ADGroup
                                .Where
                                    (p => p.Disc_ID.ToString() == selectedGroup)
                                .Select
            ...
            CommonModelGestion10ConfTQE result = new CommonModelGestion10ConfTQE
            {
                SelectedGestion10ConfTQE = adGroupName,
                AvailableToEmployeesMembersOf = availableToEmployeesMembersOf,
                Gestion10ConfTQEGroupMembersList = new List<Gestion10ConfTQEMember>()
            };
            ...
            if (!string.IsNullOrEmpty(adGroupName))
            {
                ...
                result.Gestion10ConfTQEGroupMembersList = groupmembers                    
                    .Select(m => new Gestion10ConfTQEMember { Gestion10ConfTQEGroup = adGroupName, Gestion10ConfTQEGroupID = selectedGroup, Employee = new TetraTechUser(m) }).Where(p => p.Employee.EmployeeId.Length > 0).ToList();
            }

            return result;
        }

        public List<Gestion10ConfTQEEmployeeItem> GetListEmployeeForGestion10ConfTQE(string selectedGroup)
        {
            try
            {
                var query = _entities.usp_getGestion10ConfTQEADGroupMembers(Int32.Parse(selectedGroup));

                var aList = from p in query
                            select new Gestion10ConfTQEEmployeeItem
                            {
        ...
        public OperationStatus AddEmployeeToGroupGestion10ConfTQE(string TTAccount, string discGroupID)
        {
            var discGroupName = _entities.Gestion10ConfTQE_ADGroup
                                .Where
